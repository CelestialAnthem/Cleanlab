...
def display_issues(
    ...
    *,
    ...
    output_dir: Optional[List[str]] = None,  # Parameter to save the plot image
    palette: Optional[List[List]] = None,  # Parameter to save the plot image
    **kwargs,  # Accepting additional kwargs for plt.show()
    ...
    output_dir:
        Optional directory to save the plot images. If not provided, plots will not be saved.
    ...
    palette = np.array([[r/255.0, g/255.0, b/255.0, 1.0] for r, g, b in palette])
    
    # Show a legend
    # if class_names is not None and cmap is not None:
    #     patches = [
    #         mpatches.Patch(color=palette[i], label=class_names[i]) for i in range(len(class_names))
    #     ]
        ...
        # plt.axis("off")
        # if output_dir:
        #     legend_path = f"{output_dir}/legend.png"
    ...
    for i in correct_ordering:
        ...
        if labels is not None:
            axes[plot_index].imshow(palette[labels[i]])
            axes[plot_index].set_title("Given Labels")
            plot_index += 1

        # Second image - Argmaxed pred_probs
        if pred_probs is not None:
            axes[plot_index].imshow(palette[np.argmax(pred_probs[i], axis=0)])
        ...
        if output_dir:
            fig_path = f"{output_dir}/issue_{uuid.uuid4()}.png"
